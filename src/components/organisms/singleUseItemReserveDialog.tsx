...
import { useReserveSingleUseInventoryMutation } from "@/api/inventoryApi";
import toast from "react-hot-toast";

interface SingleUseItemReserveDialogProps {
  open: boolean;
  onOpenChange: (isOpen: boolean) => void;
  itemId: string;
}

export function SingleUseItemReserveDialog({
  open,
  onOpenChange,
  itemId,
}: SingleUseItemReserveDialogProps) {
  ...
  const handleReserve = () => {
    ...
    reserveSingleUseInventoryMutation.mutate(reservationData);
  };

  const isQuantityEnabled = selectedEventId && selectedDate;
  const isReserveButtonEnabled =
    selectedEventId && selectedDate && quantity && parseInt(quantity) > 0;

  return (
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent className="sm:max-w-[425px] w-[95vw] mx-auto">
        <DialogHeader>
          <DialogTitle>Reserve Single Use Item</DialogTitle>
          <DialogDescription>
